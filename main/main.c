...
#define NEC_BIT_SYNC_HIGH_US   1 * PULSE_LENGTH_US                         /*!< NEC protocol end: positive 0.56ms */
#define NEC_BIT_SYNC_LOW_US    31 * PULSE_LENGTH_US
#define NEC_BIT_MARGIN         50                          /*!< NEC parse margin time */

#define NEC_ITEM_DURATION(d)  ((d & 0x7fff)*10/RMT_TICK_10_US)  /*!< Parse duration time from memory register value */
#define NEC_DATA_ITEM_NUM   25  /*!< NEC code item number: 24bit data + end */
#define RMT_TX_DATA_NUM  10    /*!< NEC tx test data number */
#define RMT_TIMEOUT_US 10000   /*!< RMT receiver timeout value(us) */
#define RMT_THRESHOLD_TICKS RMT_TICK_10_US * 20
...
{
    nec_fill_item_level(item, NEC_BIT_SYNC_HIGH_US, NEC_BIT_SYNC_LOW_US);
}
 ...
 */
static int nec_parse_items(rmt_item32_t* item, int item_num, uint32_t* data)
{
    int w_len = item_num;
 ...
 */
static int nec_build_items(int channel, rmt_item32_t* item, int item_num, uint32_t data)
{
    int i = 0, j = 0;
...
static void nec_rx_init()
{
    ...
    rmt_rx.rx_config.filter_ticks_thresh = RMT_THRESHOLD_TICKS;
...
static void rmt_example_nec_rx_task()
{
    ...
    while(rb) {
        ...
        if(item) {
            ...
            if(res > 0) {
                ESP_LOGI(NEC_TAG, "RMT RCV --- data: 0x%04x", rmt_data);
            }
...
static void rmt_example_nec_tx_task()
{
    ...
    for(;;) {
        ...
        while(1) {
            ...
            i = nec_build_items(channel, item + offset, item_num - offset, data);
            if(i < 0) {
                break;
            }
            data++;
