...
#define RMT_RX_GPIO_NUM  15     /*!< GPIO number for receiver */
#define RMT_CLK_DIV      100    /*!< RMT counter clock divider */
#define RMT_TICK_10_US    (80000000/RMT_CLK_DIV/100000)   /*!< RMT counter value for 10 us.(Source clock is APB clock) */

#define PULSE_LENGTH_US 350
#define NEC_HEADER_HIGH_US    1 * PULSE_LENGTH_US                         /*!< NEC protocol header: positive 9ms */
#define NEC_HEADER_LOW_US     31 * PULSE_LENGTH_US                         /*!< NEC protocol header: negative 4.5ms*/
#define NEC_BIT_ONE_HIGH_US    3 * PULSE_LENGTH_US                         /*!< NEC protocol data bit 1: positive 0.56ms */
#define NEC_BIT_ONE_LOW_US    1 * PULSE_LENGTH_US  /*!< NEC protocol data bit 1: negative 1.69ms */
#define NEC_BIT_ZERO_HIGH_US   1 * PULSE_LENGTH_US                         /*!< NEC protocol data bit 0: positive 0.56ms */
#define NEC_BIT_ZERO_LOW_US   3 * PULSE_LENGTH_US  /*!< NEC protocol data bit 0: negative 0.56ms */
#define NEC_BIT_MARGIN         20                          /*!< NEC parse margin time */

#define NEC_ITEM_DURATION(d)  ((d & 0x7fff)*10/RMT_TICK_10_US)  /*!< Parse duration time from memory register value */
#define NEC_DATA_ITEM_NUM   26  /*!< NEC code item number: header + 32bit data + end */
 ...
 */
static int nec_parse_items(rmt_item32_t* item, int item_num, uint32_t* addr, uint8_t* data)
{
    int w_len = item_num;
    if(w_len < NEC_DATA_ITEM_NUM - 1) {
        return -1;
    }
    int i = 0, j = 0;
 ...
 */
static int nec_build_items(int channel, rmt_item32_t* item, int item_num, uint32_t addr, uint8_t cmd_data)
{
    int i = 0, j = 0;
...
static void print_rx_buffer(rmt_item32_t* item, int item_num)
{
    for (int i = 0; i < item_num; i++) {
        ESP_LOGI(NEC_TAG, "%d %d %d %d", item[i].level0, item[i].duration0, item[i].level1, item[i].duration1);
    }
}
...
static void rmt_example_nec_rx_task()
{
    ...
    while(rb) {
        ...
        if(item) {
            ...
            int rec = nec_parse_items(item, num_items, &rmt_addr, &rmt_cmd);
            if (rec > 0) {
                ESP_LOGI(NEC_TAG, "addr: 0x%04x cmd: 0x%04x", rmt_addr, rmt_cmd);
            }
...
static void rmt_example_nec_tx_task()
{
    ...
    while(1) {
        ...
        nec_build_items(channel, item, item_num, addr & 0xfffff, cmd & 0xf);
